// The interaction verbs (hold / drag / balance / ratio / morse / focus / sequence) inside a timed panel.
...
const ICONS = {
  ...
  seedling: `<svg viewBox="0 0 64 64" fill="none" stroke-width="3" stroke-linecap="round"><path d="M32 58V30" stroke="#8fd18a"/><path d="M32 34c-10 0-16-6-16-14 9 0 16 5 16 14zM32 30c0-10 7-16 16-16 0 9-6 16-16 16z" fill="#3f8f45" stroke="#8fd18a"/></svg>`,
  pen: `<svg viewBox="0 0 64 64" fill="none" stroke="#c8d8f0" stroke-width="3" stroke-linejoin="round"><path d="M14 50l6-6 24-24 6 6-24 24-6 6z" fill="#8a98ad"/><path d="M44 20l6 6 4-4-6-6z" fill="#2b2f36"/><path d="M14 50l-4 4" stroke="#fff"/></svg>`,
...
const SCENES = {
  ...
  breaker: `<svg viewBox="0 0 300 180" width="100%" height="100%" aria-hidden="true"><rect x="40" y="24" width="220" height="132" rx="8" fill="#1a2230" stroke="#33445e"/>${Array.from({ length: 12 }, (_, i) => { const x = 62 + (i % 6) * 32, y = 50 + Math.floor(i / 6) * 56; return i === 8 ? `<circle cx="${x + 8}" cy="${y + 14}" r="8" fill="#000" stroke="#ffaa00" stroke-width="2"/><text x="${x + 8}" y="${y + 40}" text-anchor="middle" fill="#ffaa00" font-family="monospace" font-size="9">ENG ARM</text>` : `<rect x="${x}" y="${y}" width="16" height="28" rx="3" fill="#0b0f16"/><rect x="${x + 4}" y="${y + 4}" width="8" height="10" rx="2" fill="#6b82a3"/>`; }).join('')}<g class="placed-here"></g></svg>`,
};
...
export function mountInteraction(root, spec, { onWin = () => {}, onMiss = () => {}, timerOn = true } = {}) {
  ...
  function start() {
    ...
    const steps = spec.verb === 'sequence' ? spec.steps : [spec];
    let i = 0;
    const next = () => {
      const step = steps[i];
...
// ── HOLD: value climbs while held; release inside the zone ─────────────────────
function hold(body, spec, { hint, done }) {
  const g = spec.gauge, rate = g.zone[0] / g.riseSeconds;
  body.innerHTML = `<div class="hold-wrap">
    <div class="hold-photo" style="background-image:url('${spec.img || ''}')"><button type="button" class="hold-btn">PRESS &amp; HOLD</button></div>
    <div class="gauge" role="meter" aria-label="Temperature" aria-valuemin="0" aria-valuemax="${g.max}" aria-valuenow="0">
      <div class="zone" style="bottom:${g.zone[0] / g.max * 100}%;height:${(g.zone[1] - g.zone[0]) / g.max * 100}%"></div>
      <div class="fill"></div><div class="read">0${g.unit}</div></div></div>`;
  const btn = body.querySelector('.hold-btn'), gauge = body.querySelector('.gauge'), fill = gauge.querySelector('.fill'), read = gauge.querySelector('.read'), photo = body.querySelector('.hold-photo');
  let v = 0, held = false, last = performance.now(), raf;
  hint(`Hold to heat. Let go between ${g.zone[0]} and ${g.zone[1]}${g.unit}.`);
  const draw = () => { fill.style.height = v / g.max * 100 + '%'; read.textContent = Math.round(v) + g.unit; gauge.setAttribute('aria-valuenow', Math.round(v)); photo.style.setProperty('--glow', (v / g.max * 0.9).toFixed(2)); };
  const tick = now => {
    const dt = (now - last) / 1000; last = now;
    if (held) { v += rate * dt * SPEED; if (v >= g.max) { held = false; btn.classList.remove('down'); v = 0; hint(spec.tooHot, 'bad'); } }
    draw(); raf = requestAnimationFrame(tick);
  };
  const down = () => { held = true; btn.classList.add('down'); hint('Heating…'); };
  const up = () => {
    if (!held) return; held = false; btn.classList.remove('down');
    if (v >= g.zone[0] && v <= g.zone[1]) return done();
    if (v < g.zone[0]) { hint(spec.tooCold, 'bad'); v = 0; }
  };
  btn.addEventListener('pointerdown', e => { btn.setPointerCapture(e.pointerId); down(); });
...
// ── RATIO: one slider mixes two things; win when the ratio lands on target ─────
function ratio(body, spec, { hint, done }) {
  body.innerHTML = `<div class="ratio"><div class="flame"></div>
    <div class="mix"><span>${spec.left}</span><input type="range" min="1" max="99" step="1" value="50" aria-label="${spec.left} to ${spec.right} mix"><span>${spec.right}</span></div>
    <div class="readout" aria-live="polite"></div></div>`;
  const input = body.querySelector('input'), flame = body.querySelector('.flame'), out = body.querySelector('.readout');
  const upd = commit => {
    const h = +input.value, r = h / (100 - h), hit = Math.abs(r - spec.target) <= spec.tolerance;
    out.textContent = `${r.toFixed(2)} : 1`;
    const k = Math.min(1, Math.abs(r - spec.target) / spec.target);           // 0 = perfect
    flame.style.setProperty('--flame', hit ? '#5ab8ff' : `rgb(255,${Math.round(120 + 60 * (1 - k))},${Math.round(30 + 120 * (1 - k))})`);
    if (commit && hit) done();
  };
...
const VERBS = { hold, drag, balance, ratio, morse, focus };
